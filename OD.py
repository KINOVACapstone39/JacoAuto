...
def postprocess(frame, outs):
    ...
    for i in indices:
        ...
        width = box[2]
        height = box[3]
        globalBoxes.append([left, top, width, height])
...
def drawPred(classId, conf, left, top, right, bottom):
    ...
    label = '%.5f' % conf
    confs.append(float(label))
    # Get the label for the class name and its confidence
    if classes:
        assert (classId < len(classes))
        label = '%s:%s' % (classes[classId], label)
        id1 = '%s' %(classes[classId])
        labels.append([id1])
...
while cv.waitKey(1) < 0:
    ...
    blob = cv.dnn.blobFromImage(frame, 0.003921, (inpWidth, inpHeight), [0, 0, 0], 1, crop=False)
